...
// try to connect to mqtt server
                             ...
                             HASSFANSTATUSTOPIC, 0, 1, HASSSTATUSOFFLINEPAYLOAD))
      {
        ...
        mqttClient.subscribe(MQTTCMNDRELAY);
...
// BME280
  ...
  payload += ",\"cpuTemp\":";
  payload += String(roundf(temperatureRead() * 10) / 10);
  payload += "}";
...
void callback(char *topic, byte *payload, unsigned int length)
{
  ...
  String topicCmndRelay(MQTTCMNDRELAY);
#if defined(useOTAUpdate)
  ...
  if (topicReceived == topicCmndRelay)
  {
    if (strPayload == "on")
    {
      Log.printf("  Relay turning ON\r\n");
      relayOn();
    }
    else if (strPayload == "off")
    {
      Log.printf("  Relay turning OFF\r\n");
      relayOff();
    }
  }
